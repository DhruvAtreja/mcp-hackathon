...
import { CallToolResult } from '@modelcontextprotocol/sdk/types.js'; // Attempt to import CallToolResult
...
export type SaveMemoryToolResult = {
  structuredContent: z.infer<typeof SaveMemoryOutputPayloadSchema>;
  content?: Array<{ type: 'text'; text: string }>; // Optional textual content for LLM
};
...
export async function saveMemoryHandler(
  ...
  const user = context.req?.user || context.rawRequest?.user; // Keep trying to find user
  ...
  try {
    ...
    return {
      ...
      content: [{ type: 'text', text: "Memory saved successfully." }] // Optional convenience text
    };
  } catch (error: any) {
